...
namespace ColourCore.Handlers
{
    public class CorruptHandler : MonoBehaviour
    {
        ...
        public List<ObjectEnabler> objectEnablers = new List<ObjectEnabler>();
        private void Start()
        {
            ...
            for (int i = 0; i < objectEnablers.Count; i++)
            {
                StartCoroutine(ObjectEnabling(objectEnablers[i]));
            }
        }
        ...
        IEnumerator TriggerExplosion(CorruptExplosion data)
        {
            ...
            if (data.shake)
            {
                CameraController.singleton.Shake(.1f, .15f);
            }
        ...
        IEnumerator ObjectEnabling(ObjectEnabler data)
        {
            data.objToEnable.SetActive(false);
            yield return new WaitForSeconds(data.delay);
            data.objToEnable.SetActive(true);
            yield return new WaitForSeconds(data.duration);
            if (data.duration != 0f)
            {
                data.objToEnable.SetActive(false);
